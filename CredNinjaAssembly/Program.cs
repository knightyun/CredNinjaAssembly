...
namespace CredNinjaAssembly
{
    ...
    public class Ninja
    {
        ...
        static public string[] transformCIDR(string hosts)
        {
            string[] out_hosts = new string[10];
            ...
            if (hosts.Contains('/'))
            {
                ...
                * 192.168.0.1/24
                * 192.168.0.0-255
                * 11000000.10101000.00000000.00000000-11111111
                * 
                */

                // 192.168.0.1/24
                string[] cidr = hosts.Split('/');
                // 11000000.10101000.00000000.00000000
                string ip = cidr[0];
                int maskLen = int.Parse(cidr[1]);

                // 11111111 11111111 11111111 00000000
                uint mask = ~(uint.MaxValue >> maskLen);
                ...
                // 11000000 10101000 00000000 00000000
                uint ipFrom = BitConverter.ToUInt32(BitConverter.IsLittleEndian ? ipBytes.Reverse().ToArray() : ipBytes, 0);
                // 11000000 10101000 00000001 00000000
                uint ipTo = ipFrom + ~mask + 1;

                List<string> addresses = new List<string>();
                for (uint i = ipFrom; i < ipTo; i++)
                {
                    byte[] bytes = BitConverter.GetBytes(i);
                    IPAddress address = new IPAddress(BitConverter.IsLittleEndian ? bytes.Reverse().ToArray() : bytes);
...
";
            ...
            string[] hosts = transformCIDR(opt.hosts);
            string[] creds = opt.creds.Split(',');

            Console.WriteLine(String.Join("\n", hosts));
            //Console.WriteLine(hosts[0] + ' ' + hosts[1]);
